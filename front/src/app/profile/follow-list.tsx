    ...
    {loading ? <p className={styles.empty} role="status">목록을 불러오는 중…</p> : error ? (
      ...
      {data.users.length === 0 ? <p className={styles.empty}>{kind === "following" ? "아직 팔로우하는 사용자가 없습니다." : "아직 팔로워가 없습니다."}</p> : (
        <ul className={styles.list}>{data.users.map(user => <li key={user.userId} className={styles.row}>
          ...
          {auth.status === "authenticated" && !user.me && user.userId !== auth.user?.id && <button
            type="button"
          ...
          >{pendingId === user.userId ? "처리 중…" : user.followedByMe ? <><span className={styles.followingLabel}>Following</span><span className={styles.unfollowLabel}>Unfollow</span></> : "Follow"}</button>}
        </li>)}</ul>
      )}
