...
export default function GameSearch({ value, demo, onChange, onSearch, onSelect }: Props) {
  const listId = useId();
  ...
  const requestKey = JSON.stringify([keyword, demo]);
  ...
  }, [keyword, demo, requestKey, visible]);
  ...
  return <form className="header-search" onBlur={event => {
    ...
    <input role="combobox" aria-label="게임 제목 검색" aria-autocomplete="list" aria-expanded={visible}
      aria-controls={visible ? listId : undefined} aria-activedescendant={visible && games[active] ? `${listId}-${games[active].id}` : undefined}
      autoComplete="off" placeholder="다음 게임을 찾아보세요" value={value}
    ...
    {visible && <div className="search-dropdown">
      <ul id={listId} role="listbox" aria-label="게임 검색 후보" aria-busy={!current}>
        {games.map((game, index) => <li key={game.id} id={`${listId}-${game.id}`} role="option" aria-selected={index === active}
          ...
          <SearchCover game={game}/><div><strong>{game.title}</strong><small>{game.releaseDate?.slice(0, 4) || "출시일 미정"}</small></div>
        </li>)}
      </ul>
